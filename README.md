...
1. $\text{\boldsymbol{scDECO.cop}}$: A Gaussian copula model with flexible, covariate-dependent, and optionally zero-inflated marginals. 
2. $\text{\boldsymbol{scDECO.pg}}$: A Poisson-Gamma model with zero-inflated negative binomial marginals. Dependence between the two marginals is induced through a bivariate normal latent variable with covariate-dependent correlation parameter.
